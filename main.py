...
CLAUDE_MODEL = os.environ.get("CLAUDE_MODEL", "claude-sonnet-4-6")            # для анализа картинки — точность важна
RESEARCH_MODEL = os.environ.get("RESEARCH_MODEL", "claude-haiku-4-5-20251001")  # для текстового поиска — дешевле
...
Верни ИСПРАВЛЕННЫЙ список из {count} позиций в ТОМ ЖЕ формате (id, title, eyebrow, desc, cell, unit, tiered, color_match). Только JSON, без текста и markdown."""
...
def build_batch_research_prompt(chunk: list) -> str:
    lines = []
    for it in chunk:
...
ОБЩИЕ ПРАВИЛА:
- Используй ТОЛЬКО украинские источники (.ua, локальные шоурумы и дилеры) или международные бренды с официальной продажей в Украине. НИКОГДА не используй российские сайты (.ru, ya.ru, ozon.ru, wildberries и подобные).
- Если точного совпадения нет — всё равно найди МАКСИМАЛЬНО близкий по виду аналог, а не оставляй позицию пустой. Пустой список — только если реально ничего похожего нет.
- В поле "url" — точная ссылка на страницу САМОГО товара, не на главную сайта.
...
def research_batch(chunk: list) -> dict:
    prompt = build_batch_research_prompt(chunk)
    max_uses = min(6, max(2, len(chunk)))
...
async def handle_photo(message: Message):
    ...
    async def do_chunk(chunk):
        return await asyncio.to_thread(research_batch, chunk)

    chunk_results = await asyncio.gather(*(do_chunk(c) for c in chunks))
    data_by_id = {}
