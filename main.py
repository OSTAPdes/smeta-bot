...
ANALYSIS_PROMPT = """Ты — ассистент архитектора-дизайнера с очень внимательным глазом на детали. Перед тобой рендер интерьера или фасада, сделанный в 3ds Max.

Шаг 1. Мысленно пройдись по изображению зона за зоной (потолок, стены, пол, затем крупные предметы мебели, затем освещение, текстиль и декор) и для каждой зоны определи, что в ней находится.

Шаг 2. Для каждой обнаруженной позиции (8-14 штук) укажи:
...
- desc: 1-2 предложения. Описывай ТОЛЬКО визуально подтверждённые признаки — цвет, тип поверхности (глянец/мат/текстура), материал. Если по картинке нельзя точно отличить, например, покраску от декоративной штукатурки или от натяжного потолка — опиши нейтрально по внешним признакам ("гладкая глянцевая поверхность светлого тона"), НЕ называй конкретную технологию, если не уверен на 100%.
- x, y: координаты в процентах от ширины и высоты изображения (0-100). КРИТИЧЕСКИ ВАЖНО: координаты должны указывать ИМЕННО на сам предмет, а не рядом с ним. Перед тем как зафиксировать число — мысленно представь точку по этим координатам на картинке и проверь: она физически попадает на этот предмет (а не на пол под ним, не на стену за ним, не на соседний предмет)? Если есть малейшее сомнение — скорректируй координаты ближе к видимому центру именно этого объекта.
...
VERIFY_PROMPT_TEMPLATE = """Вот тот же рендер и черновой список позиций с координатами, который ты только что составил:

{items_json}

Сверь КАЖДУЮ позицию заново с картинкой, очень внимательно:

1. Координаты (x, y): представь точку по этим координатам на изображении. Она действительно физически находится на названном предмете (title), а не на полу под ним, не на стене за ним и не на соседнем объекте? Если нет — исправь x и y так, чтобы точка точно попадала на сам предмет.
2. Описание (desc) и название (title): соответствуют ли они тому, что реально видно? Если материал назван слишком конкретно при том, что на изображении это нельзя точно подтвердить (например написано "натяжной потолок", а по факту может быть просто глянцевая покраска или штукатурка) — перепиши нейтральнее, по видимым признакам.

Верни ИСПРАВЛЕННЫЙ список из {count} позиций в ТОМ ЖЕ формате JSON-массива с теми же полями (id, title, eyebrow, desc, x, y, unit, tiered). Никакого текста до или после JSON, никакого markdown."""
...
{{"tiers": [{{"name": "название уровня (можно пустую строку, если уровень один)", "options": [{{"name": "...", "supplier": "...", "price_label": "например '2050 ₴' или 'цена по запросу'", "price_uah": число_или_null, "avail": "в наличии / под заказ / уточнить", "url": "полная рабочая ссылка на страницу товара или каталога из результатов поиска, начинается с http"}}]}}]}}
...
def verify_items(image_b64: str, media_type: str, items: list) -> list:
    # Чистим поле data перед повторной проверкой — оно ещё не заполнено на этом этапе
    draft = [{k: v for k, v in it.items() if k != "data"} for it in items]
    ...
    try:
        ...
        log.warning("Verification pass failed to parse, using original items")
        return items
...
async def handle_photo(message: Message):
    ...
    try:
        ...
        items = await asyncio.to_thread(verify_items, img_b64, "image/jpeg", items)
